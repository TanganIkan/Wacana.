...
<body class="bg-[#d9d9d9] min-h-screen flex flex-col">
  ...
  <main class="flex-grow flex justify-center items-center px-4 relative overflow-hidden">
    ...
    <!-- Login Form Card -->
    <div id="register-form" <?= isActiveForm('register', $activeForm); ?>
      class="relative w-full max-w-md bg-gradient-to-b from-white to-[#f7f7f7] rounded-sm p-8 shadow-2xl z-10 border border-black">
      <h1 class="text-center text-[22px] font-semibold text-black mb-2">Daftar Akun</h1>
      <p class="text-center text-[15px] text-gray-600 mb-4">Masukkan detail untuk melakukan register</p>
      <form class="space-y-5" action="login-regis.php" method="post">
        <div>
          <label for="name" class="block text-[12px] font-semibold text-black mb-1">Nama*</label>
          <input id="name" name="name" type="text" required placeholder="Masukkan nama" autocomplete="name"
        ...
        <div>
          ...
          <input id="email" name="email" type="email" required placeholder="Masukkan email" autocomplete="email"
            class="w-full rounded-xm border border-black px-3 py-2 text-[14px] text-black placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-yellow-300" />
        </div>
        ...
        <div>
          <label for="confirm_password" class="block text-[12px] font-semibold text-black mb-1">Konfirmasi
            Password*</label>
        ...
        <div class="mt-6 flex flex-col sm:flex-row sm:space-x-4 space-y-3 sm:space-y-0">
          <button type="submit" name="register"
            class="bg-yellow-300 rounded-sm border border-black px-8 py-2 text-[14px] font-semibold text-black hover:brightness-90 w-full shadow-[3px_3px_0px_0px_black]">Daftar</button>
        </div>
      </form>
    </div>
  </main>
